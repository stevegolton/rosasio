...
namespace rosasio
{
    ...
    {
    public:
        Connection(boost::asio::io_context &ioc, std::map<std::string, std::function<xmlrpc_c::rpcOutcome(const xmlrpc_c::paramList &)>> &callbacks)
            : m_sock(ioc),
    ...
    private:
        ...
        std::map<std::string, std::function<xmlrpc_c::rpcOutcome(const xmlrpc_c::paramList &)>> &m_callbacks;
    };
    ...
    private:
        void start_accept()
        {
            auto conn = std::make_shared<Connection>(m_ioc, m_callbacks);

            m_acceptor.async_accept(conn->socket(),
                                    std::bind(
                                        &XmlRpcServer::accept_handler, this,
                                        conn,
                                        std::placeholders::_1));
        }

        void accept_handler(std::shared_ptr<Connection> conn, const boost::system::error_code &error)
        {
            if (!error)
            {
